...
export function Navbar() {
  ...
  const navLinks = [
    ...
    { href: `/${locale}/electoral-laws`, label: t("electoralLaws") },
      ...
      >
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex h-20 items-center justify-between">
            ...
            <Link
              href={`/${locale}`}
              className="flex items-center gap-3 sm:gap-3 group p-1 -ml-2 rounded-xl transition-all duration-300 hover:bg-white/50 active:scale-10 cursor-pointer"
              onClick={() => setMobileMenuOpen(false)}
            >
              <img
                src="/icon.svg"
                alt="دليل"
                className="h-11 sm:h-8 md:h-16 w-auto transition-all duration-300 group-hover:drop-shadow-sm"
              />
            </Link>

            {/* Desktop Navigation */}
            <div className="hidden md:flex items-center gap-2 bg-white/100 p-1.5 rounded-full border border-white/10 shadow-[inset_0_1px_4px_rgba(0,0,0,0.01)] backdrop-blur-md">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`relative px-5 py-2.5 text-sm font-semibold rounded-full transition-all duration-300 group ${isActive
                      ...
                      : "text-gray-700 hover:text-black hover:bg-black/5 hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:scale-105"
                      }`}
                  >
