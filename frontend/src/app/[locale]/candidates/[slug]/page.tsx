...
export default async function CandidateProfilePage({
  ...
  try {
    ...
    // Sources are now included in the API response
    const sources = candidate.sources || [];

  // Group statements by topic
  const statements = candidate.statements || [];
  const statementsByTopic = statements.reduce((acc: any, stmt: any) => {
    ...
    return acc;
  }, {} as Record<string, { topic: any; statements: any[] }>);
  ...
  return (
      ...
      subtitle={
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-sm sm:text-base text-gray-500">
          <Link
            ...
            className="text-cedar hover:text-cedar-light transition-colors"
          >
            {districtName}
          </Link>
          {listName && (
    ...
    >
      {/* Profile Header Card */}
      <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 shadow-sm">
        <div className="flex flex-col items-center gap-4 sm:gap-6">
          ...
          <div className="w-full grid grid-cols-2 gap-2 sm:gap-4">
            <div className="p-3 sm:p-4 bg-gray-50/50 rounded-lg sm:rounded-xl">
              ...
              <Link
                href={`/${locale}/districts/${candidate.district.id}`}
                className="text-gray-900 font-medium hover:text-cedar transition-colors text-sm sm:text-base line-clamp-2"
              >
                {districtName}
              </Link>
            </div>
      ...
      {candidate.affiliations && candidate.affiliations.length > 0 && (
        <section className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 shadow-sm">
          <h3 className="text-base sm:text-lg font-serif font-medium text-gray-900 mb-3 sm:mb-4 flex items-center gap-2">
          ...
          <div className="space-y-3 sm:space-y-4">
            {candidate.affiliations.map((aff: any) => (
              <div key={aff.id} className="pb-3 sm:pb-4 border-b border-gray-100 last:border-b-0 last:pb-0">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1 text-xs sm:text-sm text-cedar hover:text-cedar-light mt-1"
                >
      ...
      {Object.keys(statementsByTopic).length > 0 && (
        <section className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 shadow-sm">
          ...
          <div className="space-y-4 sm:space-y-6">
            {Object.values(statementsByTopic).map(({ topic, statements }: any) => (
              <div key={topic.id}>
                ...
                <div className="space-y-2 sm:space-y-3">
                  {statements.map((stmt: any) => (
                    <div key={stmt.id} className="border-l-2 border-cedar/20 pl-3 sm:pl-4 py-1">
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1 sm:mt-2 text-[10px] sm:text-xs text-gray-500">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-cedar hover:text-cedar-light truncate max-w-[200px]"
                        >
                          {stmt.source.title}
                        </a>
                      </div>
      ...
      {candidate.submissions && candidate.submissions.length > 0 && (
        <section className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 shadow-sm">
          <h3 className="text-base sm:text-lg font-serif font-medium text-gray-900 mb-1 sm:mb-2 flex items-center gap-2">
      ...
      {/* Right of reply */}
      {candidate.rightOfReplies && candidate.rightOfReplies.length > 0 && (
        <section className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 shadow-sm">
          <h3 className="text-base sm:text-lg font-serif font-medium text-gray-900 mb-3 sm:mb-4 flex items-center gap-2">
          ...
          <div className="space-y-3 sm:space-y-4">
            {candidate.rightOfReplies.map((reply: any) => (
              ...
              <li key={source.id} className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-cedar hover:text-cedar-light text-xs sm:text-sm font-medium break-words"
                  >
                    {source.title}
                  </a>
